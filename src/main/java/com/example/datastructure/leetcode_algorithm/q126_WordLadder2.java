...
Given two words (beginWord and endWord), and a dictionary's word datastructure.list, find all shortest transformation sequence(s) from beginWord to endWord, such that:

        Only one letter can be changed at a time
        Each transformed word must exist in the word datastructure.list. Note that beginWord is not a transformed word.
        For example,
...
/**
    The solution contains two steps 1 Use BFS to construct a datastructure.graph. 2. Use DFS to construct the paths from end to start.Both solutions got AC within 1s.
